...
CLI::App *TrimSilenceCommand::CreateCommandCLI(CLI::App &app) {
    auto trim_silence = app.add_subcommand(
        "trim-silence", "Trims silence from the start or end of the file(s). Silence is "
                        "considered anything under -90dB, "
                        "however this threshold can be changed with the --threshold option.");

    m_identical_processing_set.AddCli(*trim_silence);
...
std::pair<usize, usize> TrimSilenceCommand::GetLoudRegion(EditTrackedAudioFile &f) const {
...
void TrimSilenceCommand::ProcessFile(EditTrackedAudioFile &f,
...
void TrimSilenceCommand::ProcessFiles(AudioFiles &files) {
            ...
            [this](EditTrackedAudioFile *authority_file, const std::vector<EditTrackedAudioFile *> &set) {
                if (!IdenticalProcessingSet::AllHaveSameNumFrames(set)) {
                    ErrorWithNewLine(
                        ...
                        "the files in the set do not all have the same number of frames and therefore cannot be processed with trim-silence.");
                    return;
                }
...
TEST_CASE("[TrimSilenceCommand]") {
    ...
    SUBCASE("no args") {
        REQUIRE_NOTHROW(TestHelpers::ProcessBufferWithCommand<TrimSilenceCommand>("trim-silence", buf));
    }

    SUBCASE("all args") {
        REQUIRE_NOTHROW(TestHelpers::ProcessBufferWithCommand<TrimSilenceCommand>(
            "trim-silence start --threshold -90", buf));
    }

    SUBCASE("just threshold") {
        REQUIRE_NOTHROW(
            TestHelpers::ProcessBufferWithCommand<TrimSilenceCommand>("trim-silence --threshold -90", buf));
    }

    SUBCASE("trims start") {
        const auto result =
            TestHelpers::ProcessBufferWithCommand<TrimSilenceCommand>("trim-silence start", buf);
        REQUIRE(result);
    ...
    SUBCASE("trims end") {
        const auto result =
            TestHelpers::ProcessBufferWithCommand<TrimSilenceCommand>("trim-silence end", buf);
        REQUIRE(result);
    ...
    SUBCASE("trims both") {
        const auto result =
            TestHelpers::ProcessBufferWithCommand<TrimSilenceCommand>("trim-silence both", buf);
        REQUIRE(result);
