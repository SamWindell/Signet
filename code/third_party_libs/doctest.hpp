...
struct Expression_lhs {
    ...
    // comparisons, but the ExpressionDecomposer class uses the left shift operator to capture the left
...
DOCTEST_INTERFACE void
decomp_assert(assertType::Enum at, const char *file, int line, const char *expr, Result result);
...
struct DOCTEST_INTERFACE IReporter {
    ...
    // called when a test case is reentered because of unfinished subcases (safe to cache a pointer to the
    // input)
    ...
    // doctest will not be managing the lifetimes of reporters given to it but this would still be nice to
    // have
    virtual ~IReporter();
    ...
    // can iterate through contexts which have been stringified automatically in their destructors when an
...
DOCTEST_INTERFACE void
registerReporterImpl(const char *name, int prio, reporterCreatorFunc c, bool isReporter);
        ...
        iter(const char *file, unsigned line, int index) {                                                   \
            doctest::detail::regTest(                                                                        \
                doctest::detail::TestCase(                                                                   \
                    func<Type>, file, line, doctest_detail_test_suite_ns::getCurrentTestSuite(),             \
...
#define DOCTEST_BINARY_ASSERT(assert_type, comp, ...)                                                        \
    do {                                                                                                     \
        doctest::detail::ResultBuilder _DOCTEST_RB(doctest::assertType::assert_type, __FILE__, __LINE__,     \
                                                   #__VA_ARGS__);                                            \
        DOCTEST_WRAP_IN_TRY(                                                                                 \
            _DOCTEST_RB.binary_assert<doctest::detail::binaryAssertComparison::comp>(__VA_ARGS__))           \
        DOCTEST_ASSERT_LOG_AND_REACT(_DOCTEST_RB);                                                           \
    } while ((void)0, 0)
...
#define DOCTEST_UNARY_ASSERT(assert_type, ...)                                                               \
    doctest::detail::unary_assert(doctest::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__,        \
                                  __VA_ARGS__)
...
struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats {
    ...
    void finalizeTestCaseData() {
        ...
        if (failure_flags && !ok_to_fail) numTestCasesFailed++;
    }
};
...
Subcase::Subcase(const String &name, const char *file, int line) : m_signature({name, file, line}) {
    ...
    if (s->subcasesStack.size() < size_t(s->subcase_filter_levels)) {
        if (!matchesAny(m_signature.m_name.c_str(), s->filters[6], true, s->case_sensitive)) return;
        if (matchesAny(m_signature.m_name.c_str(), s->filters[7], false, s->case_sensitive)) return;
    }
...
Subcase::~Subcase() {
    if (m_entered) {
        ...
        if (g_cs->should_reenter == false) g_cs->subcasesPassed.insert(g_cs->subcasesStack);
...
std::ostream &file_line_to_stream(std::ostream &s, const char *file, int line, const char *tail = "") {
...
bool parseIntOption(int argc, const char *const *argv, const char *pattern, optionType type, int &res) {
    ...
    if (type == 0) {
        ...
        // integer
        // TODO: change this to use std::stoi or something else! currently it uses undefined behavior -
        // assumes '0' on failed parse...
...
#ifdef DOCTEST_PLATFORM_WINDOWS
    if (isDebuggerActive()) p->reporters_currently_used.push_back(new DebugOutputWindowReporter(*g_cs));
#endif // DOCTEST_PLATFORM_WINDOWS
    ...
    for (auto &curr : testArray) {
        ...
        // execute the test if it passes all the filtering
        {
            ...
            do {
                ...
                if (p->should_reenter && run_test) DOCTEST_ITERATE_THROUGH_REPORTERS(test_case_reenter, tc);
