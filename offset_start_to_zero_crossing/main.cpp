...
AudioFile ReadAudioFile(const std::string &filename) {
    ...
    if (ext == ".wav") {
        ...
        const auto frames_read = drwav_read_pcm_frames_f32(&wav, wav.totalPCMFrameCount, result.interleaved_samples.data());
        ...
        const auto frames_read = drflac_read_pcm_frames_f32(flac, flac->totalPCMFrameCount, result.interleaved_samples.data());
...
bool WriteWaveFile(const std::string &filename, const AudioFile &audio_file) {
    ...
    const auto num_written = drwav_write_pcm_frames(wav, audio_file.NumFrames(), audio_file.interleaved_samples.data());
